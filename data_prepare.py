...
parser.add_argument('--dataset', type=str, default='CNSig', help='dataset name, options: [CNSig, CEDAR, BHSig-B, BHSig-H]')
args = parser.parse_args()
if args.dataset == 'CNSig':
    num_people = 836 # number of writers
    true_times = 10  # number of real signatures
    false_times = 10 # number of forged signatures
elif args.dataset == 'CEDAR':
...
if args.dataset == 'CNSig':
    false_data_path = 'datasets/%s/false_change'%args.dataset
else:
...
def get_information(data):
    ...
    if y == 1:
        if args.dataset == 'CNSig':
            x1 = os.path.join(true_data_path, names[writer_id], f'{x1}.png')
            x2 = os.path.join(true_data_path, names[writer_id], f'{x2}.png')
        else:
        ...
        if args.dataset == 'CNSig':
            x1 = os.path.join(true_data_path, names[writer_id], f'{x1}.png')
            x2 = os.path.join(false_data_path, names[writer_id], f'{x2}.png')
        else:
            x1 = os.path.join(true_data_path, 'original_' + str(writer_id + 1) + f'_{x1 + 1}.png')
