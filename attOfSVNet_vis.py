...
class Encoder(torch.nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = torch.nn.Conv2d(args.num_channels, 32, 3, stride=1, padding=1)
...
class Attention2D(torch.nn.Module):
    def __init__(self, input_size, hidden_size, args, index):
    ...
    def forward(self, img_fvec, patch_fmap):
        ...
        img = torchvision.transforms.ToPILImage()((torch.mean(x_em, 1) * 255).byte())
        if not os.path.exists('vis/%s/outputs'%self.dataset):
            os.makedirs('vis/%s/outputs'%self.dataset)
        if self.inv:
            img.save('vis/%s/outputs/inv_%s.jpg'%(self.dataset, self.index))
        else:
            img.save('vis/%s/outputs/non-inv_%s.jpg'%(self.dataset, self.index))
...
class Decoder(torch.nn.Module):
    def __init__(self, args):
        ...
        self.att1 = Attention2D(128, 64, args, 1)
        self.att2 = Attention2D(64, 32, args, 2)
        self.att3 = Attention2D(32, 16, args, 3)
